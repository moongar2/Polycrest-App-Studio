            ...
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (result.status == "OK") { // We're running!
                        ...
                        pc = 1;                    // Next State
                        if (usb) {
                            counter = 1;               // Fast connect for USB
                        } else {
                            counter = 2;              // Fast connect for Emulator
                        }
...
Blockly.ReplMgr.makeDialogMessage = function(code) {
    ...
    var retval = '<div style="font-family: Outfit, sans-serif; max-width: 440px;">'
        + '<table style="width: 100%; margin-bottom: 12px;"><tr>'
        + '  <td style="vertical-align: middle; padding-right: 14px;">' + img + '</td>'
        + '  <td style="vertical-align: middle;">'
        + '    <div style="font-size: 13px; color: #94A3B8; margin-bottom: 4px;">' + Blockly.Msg.REPL_YOUR_CODE_IS + ':</div>'
        + '    <div style="font-size: 26px; font-weight: 800; letter-spacing: 2px; color: #00D4FF; font-family: monospace;">' + code + '</div>'
        + '    <div style="font-size: 11px; color: #64748B; margin-top: 6px;">Open PolyCrest Companion on your phone and type this code or scan the QR code.</div>'
        + '  </td>'
        + '</tr></table>'
        + '<div style="background: rgba(15, 23, 42, 0.85); border: 1px solid rgba(0, 212, 255, 0.3); border-radius: 10px; padding: 10px 14px; margin-top: 8px; display: flex; flex-direction: column; gap: 6px; text-align: center;">'
        + '  <div style="font-size: 12px; font-weight: bold; color: #F8FAFC;">Need the App or Desktop Bridge?</div>'
        + '  <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">'
        + '    <a href="/appstudio/PolyCrestCompanion.apk" download style="display: inline-block; padding: 6px 12px; background: linear-gradient(135deg, #0D99FF, #00D4FF); color: #000; font-weight: bold; border-radius: 6px; text-decoration: none; font-size: 11.5px;">📥 Download Companion APK</a>'
        + '    <a href="/appstudio/" target="_blank" style="display: inline-block; padding: 6px 12px; background: #1E293B; color: #38BDF8; border: 1px solid #00D4FF; border-radius: 6px; font-weight: bold; text-decoration: none; font-size: 11.5px;">⚡ Get PCstarter</a>'
        + '  </div>'
        + '</div>'
        + '</div>';
    return retval;
