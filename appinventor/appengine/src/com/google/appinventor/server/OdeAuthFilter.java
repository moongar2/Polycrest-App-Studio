...
//    }
  ...
  public static class UserInfo implements Serializable {
    ...
    boolean isValid() {
      ...
      if (offset < -86400 || offset > (60*idleTimeout.get())) {
        return false;
      } else {
        return true;
      }
  ...
  public static UserInfo getUserInfo(HttpServletRequest request) {
    try {
      ...
      LOG.log(Level.SEVERE, "Error parsing cookie: " + e.getMessage(), e);
      return null;
    }
  }
  ...
  private static Crypter getCrypter() throws KeyczarException {
    synchronized(crypterSync) {
      if (crypter != null) {
        ...
        String keyPath = sessionKeyFile.get();
        java.io.File keyFile = new java.io.File(keyPath);
        if (!keyFile.exists()) {
          java.io.File inWar = new java.io.File("war", keyPath);
          if (inWar.exists()) {
            keyPath = inWar.getAbsolutePath();
          } else {
            java.io.File inEngineWar = new java.io.File("engine/war", keyPath);
            if (inEngineWar.exists()) {
              keyPath = inEngineWar.getAbsolutePath();
            } else {
              java.io.File inBuildWar = new java.io.File("appengine/build/war", keyPath);
              if (inBuildWar.exists()) {
                keyPath = inBuildWar.getAbsolutePath();
              }
            }
          }
        }
