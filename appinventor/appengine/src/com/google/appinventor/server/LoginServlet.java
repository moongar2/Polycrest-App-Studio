...
public class LoginServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    if (reqUri.contains("/static/") || reqUri.endsWith(".css") || reqUri.endsWith(".js") || reqUri.endsWith(".png") || reqUri.endsWith(".ico") || reqUri.endsWith(".svg") || reqUri.endsWith(".gif") || reqUri.endsWith(".jpg") || reqUri.endsWith(".jpeg") || reqUri.endsWith(".woff2") || reqUri.endsWith(".ttf")) {
      String staticPath = reqUri.contains("/static/") ? reqUri.substring(reqUri.indexOf("/static/")) : "/" + reqUri.substring(reqUri.lastIndexOf('/') + 1);
      try {
        req.getRequestDispatcher(staticPath).forward(req, resp);
        return;
...
//      req.getSession().setAttribute("userid", user.getUserId()); // This effectively logs us in!
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    User user = storageIo.getUserFromEmail(email.trim());
    boolean validLogin = false;
    String hash = user.getPassword();

    if ("google".equalsIgnoreCase(provider) || "firebase".equalsIgnoreCase(provider)) {
      if ((hash == null || hash.isEmpty()) && password != null && !password.isEmpty()) {
        try {
          storageIo.setUserPassword(user.getUserId(), PasswordHash.createHash(password));
