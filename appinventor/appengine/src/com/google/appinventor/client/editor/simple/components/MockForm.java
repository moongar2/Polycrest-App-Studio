...
public final class MockForm extends MockDesignerRoot implements DesignerRootComponent {
  ...
  private void setPhoneStyle() {
    boolean isTablet = (idxPhoneSize >= 3 && idxPhoneSize <= 4);
    boolean isMonitor = (idxPhoneSize == 5);
    if (landscape) {
      if (isTablet) {
  ...
  private void setIOSPhoneStyle() {
    int sizeIndex = (idxPhoneSize >= 3 && idxPhoneSize <= 4) ? 1 : (idxPhoneSize == 5 ? 2 : 0);
    if (landscape) {
      phoneWidget.setStylePrimaryName(iosSizesLandscape[sizeIndex]);
    } else {
      phoneWidget.setStylePrimaryName(iosSizesPortrait[sizeIndex]);
    }
    navigationBar.setStylePrimaryName("ode-SimpleMockFormNavigationBarIOS");
    phoneBar.setIconColor(blackIcons, sizeIndex);
    phoneBar.setSize(sizeIndex);
  }
